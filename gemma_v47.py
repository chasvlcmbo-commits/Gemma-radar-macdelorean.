...
st.set_page_config(page_title="GEMA v62 TOTAL ARSENAL", page_icon="🦅", layout="wide")
...
universos = {
    ...
    "🚜 RUSSELL 2000 (SELECT)": ["IWM", "TNA", "URTY", "SMH", "SOXX", "XBI", "KWEB", "EWZ", "GDX", "GDXJ", "SIL", "TAN", "ICLN", "PBW", "MSTR", "ELF", "CROX", "CVNA", "UPST", "AI", "IONQ", "JOBY", "QS", "SPCE", "OPEN", "NKLA", "SAVA", "GME", "AMC", "TLRY", "OXY", "HAL", "SLB", "APA", "DVN", "MRO", "HES", "COP", "EOG", "VLO", "MPC", "PSX", "KMI", "WMB", "ET", "EPD", "MPLX", "PAA", "VNOM", "OIH", "XOP", "AMLP", "USO", "UNG", "BOIL", "KOLD", "GUSH", "NUGT", "DUST", "JNUG", "JDST", "LIT", "URA", "REMX", "COPX", "SLX", "FAN", "GRID", "BATT", "DRIV", "KARS", "HAIL", "IDRV", "VCAR", "UWMC", "RKT", "LDI", "PFSI", "GHIV", "IVR", "MFA", "NYMT", "MITT", "TWO", "ARR", "CIM", "EARN", "ORC", "AGNC", "NLY", "STWD", "BXMT", "ABR", "RC", "GPMT", "KREF", "LADR", "TRTX", "ACRE", "ARI", "XAN", "CLNC", "HMG", "HASI", "SAFE", "STAR", "GTY", "NNN", "O", "WPC", "STAG", "ADC", "EPR", "VICI", "GLPI", "MGP", "IRT", "MAA", "CPT", "ESS", "AVB", "EQR", "UDR", "AIV", "INVH", "AMH", "TCON", "UMH", "SUI", "ELS", "CUBE", "EXR", "PSA", "LSI", "NSA", "COLD", "DLR", "EQIX", "COR", "CONE", "QTS", "AMT", "CCI", "SBAC", "UNIT", "LAMR", "OUT", "WY", "PCH", "RYN", "CTT", "LAND", "FPI", "AGM"]
}

# ==============================================================================
# 2. MOTOR TÉCNICO (CLON v22)
# ==============================================================================
def check_vela_v22(df, idx=-1):
    ...
    # Lógica v22 Pura: Barrido + Cierre > Mitad anterior + Estocástico < 20 (Compra) o > 80 (Venta)
...
def analizar_total_war(ticker, strategy):
    try:
        # Descarga multitemporal idéntica al robot v22
        ...
        # Procesar indicadores estilo v22
        ...
        if strategy == "💎 PREMIUM (Cerebro v22)":
            cm = df_m.iloc[-1]; pm = df_m.iloc[-2]
            # Filtro Mensual Estricto v22
            m_bull = (cm['MACD'] > 0) and (cm['MACD'] > cm['Signal']) and (cm['MACD'] > pm['MACD'])
            m_bear = (cm['MACD'] < 0) and (cm['MACD'] < cm['Signal']) and (cm['MACD'] < pm['MACD'])
            
            # Buscamos la vela en las últimas 5 semanas
            for i in range(5):
                ...
                if m_bear and es and "BAJISTA" in tipo:
                    return {"Ticker": ticker, "Señal": f"SELL PREMIUM (Hace {i} sem)", "Precio": precio, "Stoch": round(k,1)}
        else:
            # Modo Vela de Engaño Simple: revisa últimas 4 semanas
            for i in range(4):
                es, tipo, k = check_vela_v22(df_w, idx=-1-i)
                if es: return {"Ticker": ticker, "TF": "W", "Señal": tipo, "Hace": f"{i} sem", "Precio": precio, "Stoch": round(k,1)}
        return None
    except: return None

# --- UI ---
st.title("🦅 GEMA v62 - TOTAL WAR ARSENAL")
c1, c2 = st.columns(2)
with c1:
    idx = st.selectbox("📂 Escuadrón Seleccionado:", list(universos.keys()))
with c2:
    estrat = st.selectbox("🧠 Estrategia:", ["💎 PREMIUM (Cerebro v22)", "🕯️ VELAS DE ENGAÑO"])
...
if st.button("🔥 LANZAR RADAR"):
    ...
    # 5 workers para no saturar a Yahoo y obtener datos reales
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(analizar_total_war, t, estrat): t for t in activos}
    ...
    if encontrados:
        ...
        st.warning("Prueba con el modo 'VELAS DE ENGAÑO' o cambia de índice.")
